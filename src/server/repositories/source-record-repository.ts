...
export function addWorkNote(
  ...
  try {
    ...
    if (!sourceRecord.isValidRecord()) {
      gs.error(`[IDV WorkNote] Record not found. table=${sourceTable}, sys_id=${sourceRecordId}`);
      return false;
    }
    ...
    // Journal fields can't use setValue()/setJournalEntry() in scoped apps - direct field assignment is the supported approach.
    (sourceRecord as any).work_notes = note;
    const updatedId = sourceRecord.update();

    if (!updatedId) {
      gs.error(`[IDV WorkNote] Update returned no sys_id. table=${sourceTable}, sys_id=${sourceRecordId}`);
      return false;
    }

    gs.info(`[IDV WorkNote] Work note added. table=${sourceTable}, sys_id=${sourceRecordId}`);
    return true;
  } catch (error) {
    gs.error(
      `[IDV WorkNote] Failed to add work note. table=${sourceTable}, sys_id=${sourceRecordId}, error=${error}`,
    );
    return false;
  }
}