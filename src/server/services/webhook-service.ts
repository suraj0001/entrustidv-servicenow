...
import * as configurationRepository from '../repositories/configuration-repository.ts';
...
function processWorkflowRunCompleted(payload: EntrustWebhookPayload): void {
  const workflowRunId =
    payload.resource?.id ||
    payload.object?.id ||
  ...
  const status = payload.resource?.status || payload.object?.status;

  const eventWorkflowId = payload.resource?.workflow_id || payload.object?.workflow_id;
  ...
  if (eventWorkflowId && config?.workflowId && eventWorkflowId !== config.workflowId) {
    gs.info(`[EntrustWebhook] Ignoring event for unrelated workflow_id=${eventWorkflowId}`);
    return;
  }

  const verificationRequest =
    verificationRequestRepository.findVerificationRequestByWorkflowRunId(workflowRunId);

  if (!verificationRequest) {
    gs.warn(`[EntrustWebhook] No verification request found for workflowRunId=${workflowRunId}`);
    return;
  }
  ...
  const isAlreadyTerminal = ['approved', 'declined', 'review', 'abandoned', 'error'].includes(
    currentNormalized
  );
  ...
  verificationRequestRepository.updateStatusByWorkflowRunId(workflowRunId, status);
...
function processEvidenceFolderCreated(payload: EntrustWebhookPayload): void {
  const workflowRunId =
    payload.object?.workflow_run_id ||
    payload.resource?.workflow_run_id ||
    payload.object?.id ||
  ...
  const evidenceFolderHref = payload.object?.href || payload.resource?.href;
  ...
  gs.info(`[EntrustWebhook] Evidence folder recorded for workflowRunId=${workflowRunId}`);
}
