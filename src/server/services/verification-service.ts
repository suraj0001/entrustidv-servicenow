...
export function startVerification(
  ...
  gs.info(
    `[VerificationService] startVerification: sourceTable=${sourceTable}, sourceRecordId=${sourceRecordId}`
  );
  ...
  gs.info(
    `[VerificationService] sourceContext resolved: sourceTable=${sourceContext.sourceTable}, sourceRecordId=${sourceContext.sourceRecordId}, subjectUserId=${sourceContext.subjectUserId}`
  );
  ...
  if (existingRequestCount >= MAX_VERIFICATION_REQUESTS) {
    throw new Error(RESPONSE_MESSAGES.MAX_REQUESTS_REACHED(MAX_VERIFICATION_REQUESTS));
  }
  ...
  if (applicantId) {
    gs.info(
      `[VerificationService] Reusing existing applicantId=${applicantId} for subjectUserId=${sourceContext.subjectUserId}`
    );
  } else {
    ...
    gs.info(
      `[VerificationService] New Entrust applicant created: applicantId=${applicantId} for subjectUserId=${sourceContext.subjectUserId}`
    );
  }
  ...
  gs.info(
    `[VerificationService] Workflow run created: workflowRunId=${workflowRun.workflowRunId}, status=${workflowRun.status}, workflowVersionId=${workflowRun.workflowVersionId}`
  );

  // Persist only after Entrust confirms the workflow run was created
  deactivateActiveVerificationRequests(sourceContext.sourceTable, sourceContext.sourceRecordId);
  ...
  gs.info(
    `[VerificationService] Verification request persisted: verificationRequestId=${verificationRequestId}`
  );
  ...
  gs.info(
    `[VerificationService] Event queued: ${VERIFICATION_REQUEST_CREATED_EVENT}, returning status=${workflowRun.status}`
  );
