...
export const ACTIVITY_MESSAGES = {
  ...
  REVERIFICATION_REQUESTED:
    'Identity reverification requested. Verification link sent to the user.',
  REVERIFICATION_REQUESTED_AGAIN:
    'Identity reverification requested again. Verification link sent to the user.',
  ...
  STALE_WORKFLOW_RUN:
    '⚠ A previous verification link was used. Please ask the user to use the latest verification link.',
} as const;
...
export const RESPONSE_MESSAGES = {
  ...
  MAX_REQUESTS_REACHED: (max: number) =>
    `Maximum number of identity verification requests (${max}) has been reached for this record.`,
} as const;
