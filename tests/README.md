...
| :--- | :--- | :--- |
| `tc_api_connection_tests.js` | **Admin - API Connection Tests** | - Missing parameter validation<br>- Unsupported region rejection (`invalid_region`)<br>- Invalid credential auth failure against Entrust API<br>- `getConfig()` retrieval<br>- `saveConfig()` validation |
| `tc_verification_settings_tests.js` | **Admin - Verification Settings Tests** | - Missing Workflow ID error<br>- Positive integer Link Expiry validation<br>- Valid HTTP/HTTPS Redirect URL check<br>- Save & retrieve valid verification settings<br>- Webhook Token secret length validation (5-100 chars, used by `setup-information.html`) |
