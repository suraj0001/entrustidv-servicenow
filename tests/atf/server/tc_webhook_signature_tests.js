...
(function (outputs, steps, params, stepResult, assertEqual) {
  ...
  var validator =
    typeof EntrustWebhookSignatureValidator !== 'undefined'
  ...
  var samplePayload =
    '{"event":"workflow_run.completed","workflow_run_id":"wfr_test_123","status":"Approved"}';
  ...
  check(
    'Should fail when signature header is empty',
    validator.validate(samplePayload, '', sampleSecret),
    false
  );
  check(
    'Should fail when webhookSecret is empty',
    validator.validate(samplePayload, 'sig', ''),
    false
  );
  ...
  check(
    'Should fail when signature length is not 64 chars',
    validator.validate(samplePayload, 'short_invalid_sig', sampleSecret),
    false
  );

  var nonHexSig = 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz'; // 64 non-hex chars
  check(
    'Should fail when signature is non-hex',
    validator.validate(samplePayload, nonHexSig, sampleSecret),
    false
  );
  ...
  var isValidUppercase = validator.validate(
    samplePayload,
    validHexSignature.toUpperCase(),
    sampleSecret
  );
  ...
  var tamperedPayload =
    '{"event":"workflow_run.completed","workflow_run_id":"wfr_test_123","status":"Declined"}';
  ...
  var isWrongSecretValid = validator.validate(
    samplePayload,
    validHexSignature,
    'a_completely_different_secret'
  );
  check(
    'Signature validation should fail when the secret does not match',
    isWrongSecretValid,
    false
  );
