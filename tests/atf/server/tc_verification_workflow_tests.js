...
(function(outputs, steps, params, stepResult, assertEqual) {
    ...
    check("countVerificationRequests should count all requests regardless of active state", repo.countVerificationRequests(mockSourceTable, mockSourceSysId), 2);
    check("countVerificationRequests should return 0 for a source record with no requests", repo.countVerificationRequests(mockSourceTable, "no_such_source_record"), 0);
    ...
    check("findVerificationStatusByWorkflowRunId should return null for an unknown workflow run ID", repo.findVerificationStatusByWorkflowRunId("no_such_wfr_id"), null);
    ...
    if (grVerifySync.get(createdSysId)) {
        check("last_status_sync should be updated to the given timestamp", grVerifySync.getValue("last_status_sync"), syncTimestamp);
    } else {
