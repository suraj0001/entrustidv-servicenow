...
(function (outputs, steps, params, stepResult, assertEqual) {
  ...
  webhookSvc.processWebhook({
    payload: { action: 'some_unhandled_event', resource: { id: 'whatever' } },
  });
  ...
  if (grVerifyStale.get(staleSysId)) {
    check(
      "Stale request status should remain 'Pending' and not update",
      grVerifyStale.getValue('status'),
      'Pending'
    );
  }
  ...
  webhookSvc.processWebhook({
    payload: { action: 'workflow_run.completed', resource: { id: 'wfr_missing_status' } },
  });
  ...
  if (grVerifyActive.get(activeSysId)) {
    check(
      "Active request status should update to 'Approved'",
      grVerifyActive.getValue('status'),
      'Approved'
    );
  }
  ...
  if (grVerifyIdempotent.get(activeSysId)) {
    check(
      "Status already terminal ('Approved') should not be overwritten by a later webhook",
      grVerifyIdempotent.getValue('status'),
      'Approved'
    );
  }
  ...
  if (grVerifyAltShape.get(altShapeSysId)) {
    check(
      'object.* payload shape should update status just like resource.*',
      grVerifyAltShape.getValue('status'),
      'Declined'
    );
  }
  ...
  var configuredWorkflowId =
    savedSettings && savedSettings.settings ? savedSettings.settings.workflowId : null;
  ...
  if (configuredWorkflowId) {
    ...
    webhookSvc.processWebhook({
      payload: {
        ...
        resource: {
          id: mismatchWfrId,
          status: 'Approved',
          workflow_id: configuredWorkflowId + '_unrelated',
        },
      },
    });
    ...
    if (grVerifyMismatch.get(mismatchSysId)) {
      check(
        'Event for an unrelated workflow_id should be ignored',
        grVerifyMismatch.getValue('status'),
        'Pending'
      );
    }
  } else {
    gs.warn(
      '[ATF TEST 6.3d] Skipped: no workflow_id currently configured to test mismatch rejection against.'
    );
  }
  ...
  if (grVerifyEvidence.get(activeSysId)) {
    check(
      'Evidence folder href should be recorded',
      grVerifyEvidence.getValue('evidence_folder_href'),
      'https://api.us.idv.entrust.com/v2.0/evidence_folders/folder_12345'
    );
  }
  ...
  webhookSvc.processWebhook({
    payload: { action: 'workflow_run_evidence_folder.created', object: {} },
  });
  webhookSvc.processWebhook({
    payload: {
      action: 'workflow_run_evidence_folder.created',
      object: { workflow_run_id: activeWfrId },
    },
  });
  ...
  if (grVerifyEvidenceAlt.get(activeSysId)) {
    check(
      'resource.* payload shape should update the evidence folder href',
      grVerifyEvidenceAlt.getValue('evidence_folder_href'),
      'https://api.us.idv.entrust.com/v2.0/evidence_folders/folder_67890'
    );
  }
  ...
  // -------------------------------------------------------------------------
  gs.info(
    '[ATF TEST 6.4d] Testing redundant evidence folder event with the same href is a no-op...'
  );
  webhookSvc.processWebhook({
    payload: {
      action: 'workflow_run_evidence_folder.created',
  ...
  if (grVerifyEvidenceIdempotent.get(activeSysId)) {
    check(
      'Re-delivering the same evidence folder href should be a no-op',
      grVerifyEvidenceIdempotent.getValue('evidence_folder_href'),
      'https://api.us.idv.entrust.com/v2.0/evidence_folders/folder_67890'
    );
  }
  ...
  webhookSvc.processWebhook({
    payload: {
      action: 'workflow_run_evidence_folder.created',
      object: {
        workflow_run_id: 'wfr_no_such_request_' + gs.generateGUID(),
