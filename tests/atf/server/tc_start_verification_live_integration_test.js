...
(function(outputs, steps, params, stepResult, assertEqual) {
    ...
    var connectionReady = !!(connectionConfig && connectionConfig.success && connectionConfig.connectionTested);
    var workflowReady = !!(settingsConfig && settingsConfig.success && settingsConfig.settings && settingsConfig.settings.workflowId);
    ...
    if (!connectionReady || !workflowReady) {
        ...
        stepResult.setOutputMessage("Skipped: real Entrust API connection and/or workflow ID are not configured on this instance.");
        return true;
    }
    ...
    // -------------------------------------------------------------------------
    gs.info("[ATF TEST 6.1] Calling startVerification() against the real, configured Entrust connection...");
    ...
    try {
        ...
        check("startVerification should return a display status of In Progress", result.displayStatus, "In Progress");
        ...
        if (persistedFound) {
            check("Persisted record should reference the returned workflow run ID", persistedGr.getValue("workflow_run_id"), result.workflowRunId);
        }

        stepResult.setOutputMessage("LIVE Entrust integration test completed successfully. workflowRunId=" + result.workflowRunId);
    } finally {
